...
/**
 * This class maintains the record of all generated events in a log file. There
 * are actually two log files - an 'actual' log file and a 'backup' log file. If
 * the size of the actual log file exceeds 100K bytes, then the contents of the
 * actual log are transferred to the backup log, removing any previously
 * existing backup log. A new actual log file is started. The format of a
 * typical log file entry is as follows:
 *
 * [10/27/00 11:17
 * AM]|DatabaseAccessor|getMultiValuedSelectResult|Error|EVT_SQLException|null<The insert query could not be successfully completed>
 *
 * Each of the sub-entries for each log file entry is separated from the
 * previous by a separator (|). We first have the date and time of the event,
 * followed by an indication of the class in which the Event was generated,
 * followed by the name of the method in which it was generated, followed by the
 * Severity level description, followed by the tag which points to the
 * Internationalized description, followed by the (possibly null) list of
 * parameters (which if it exists will be a comma-separated string), followed by
 * the plain English description enclosed in
 * <...>. NOTE THAT THIS FORMAT IS IMPORTANT AND ANY CHANGE IN IT WILL AFFECT
 * additional objects that parse this string
 *
 * This class implements the SINGLETON pattern. Hence, there will only be one
...
public class EventLog {
    ...
    /**
     * Param length limit - to prevent long file names from stretching windows
     * too long
     */
    ...
    /**
     * Flag that turns off/on the actual writing of the log. Sometimes it is
     * necessary to disable writing (e.g., if the app using this class is
     * running off a CD, or is running from an applet)
     */
    private static boolean allowWrites = true;

    /**
     * Flag that turns off/on the popup message to show the error. Will be
     * turned off if this is running on the server side (for example).
     */
    private static boolean allowPopupDisplay = false;

    /**
     * Create and return an instance of EventLog. This method enforces a
     * Singleton pattern.
     *
    ...
    /**
     * Set the messages bundle so we can translate the tags in the event content
     * and display the translated description
     *
    ...
    public void addEvent(Event event) {
        ...
        if ((event.getSeverityDesc().equals("Warning") == true)
            ...
            if (allowPopupDisplay == true) {
                ...
                } else // if there are translations available, but this event doesn't support them, use raw text
    ...
    /**
     * This method exists to enable the programmer to indicate whether
     * sub-errors in the parameter processing should be displayed or not, via
     * the third boolean parameter. This method, by default, checks for long
     * file names. If this check is to be turned off, then the four-parameter
     * version of this method has to be used
     *
     * @param	source	The actual description of the event (with embedded
     * parameter points) that has to be displayed in the popup
     *
     * @param	paramList	The comma-separated String indicating the parameter list
     * that is used to substitute the parameter points in the original
     * description
     *
     * @param	displaySubErrors	boolean indicating whether to display sub-errors
     * (generated in parsing description string) or not
     *
     */
    //---------------------------------------------------------------
    ...
    /**
     * This method exists because it has to handle the fact that
     * replaceParameters is called when the Error Log table is redisplayed in
     * EasyConfig as well. The invocation of replaceParameters in this context
     * should not put up any popups for any internal errors it finds - hence, we
     * invoke the other replaceParameters directly from here, passing it a flag
     * saying we do not want any sub-error popups. This method, which has only
     * two parameters, is the default invocation which arranges for sub-error
     * popups to be displayed. Note that this checks for long file names as well
     *
     * @param	source	The actual description of the event (with embedded
     * parameter points) that has to be displayed in the popup
     *
    ...
    /**
     * This method maps parameter point placeholders in internationalized
     * description strings to parameters. It has a boolean parameter that
     * indicates whether any sub-errors generated by this method (in parsing the
     * description string) should be displayed or not
     *
     * @param	source	The actual description of the event (with embedded
     * parameter points) that has to be displayed in the popup
     *
     ...
     * (generated in parsing description string) or not
     *
     * @param	checkFileLength	boolean indicating whether to check for long file
     * names or not
     */
    //-------------------------------------------------------------
    public static String replaceParameters(String source, String paramList, boolean displaySubErrors,
